...
def IniciarServidor():
    ...
    def get_usuarios():
        file_path = os.path.join(BASE_DIR, './arquivos json/usuarios.json')
        try:
    ...
    def get_carros():
        file_path = os.path.join(BASE_DIR, './arquivos json/veiculos.json')
        try:
            with open(file_path, 'r') as file:
                data = json.load(file)
            return jsonify(data)
    ...
    def get_alugados():
        file_path = os.path.join(BASE_DIR, './arquivos json/alugados.json')
        try:
            with open(file_path, 'r') as file:
                data = json.load(file)
            return jsonify(data)
...
if opcao == 1:
    ...
    # Atenção há um bug ao iniciar o servidor, que e necessário digitar a opção 2 duas vezes para ele inciar corretamente
    IniciarServidor()
